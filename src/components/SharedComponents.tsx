...
export function TaskItem({ task, isAdmin, onStart, onComplete, onStop, onDelete, onAssign, depth = 0 }: any) {
    ...
    return (
        <div className="transition-all duration-300">
            <div className={clsx("group p-5 flex items-center justify-between hover:bg-[var(--card-hover)]", 
                depth > 0 && "border-l border-[var(--border)] bg-[var(--card)]/50",
                { 'pl-10': depth === 1, 'pl-16': depth === 2 }
            )}>
                <div className="flex items-center gap-4">
                    {hasChildren && (
                        <button onClick={() => setExpanded(!expanded)} className="p-1 hover:bg-[var(--border)] rounded">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                             <h4 className={clsx("font-bold text-[var(--foreground)] group-hover:text-blue-500 transition-colors", depth > 0 && "text-sm")}>{task.title}</h4>
                        ...
                        <div className="flex items-center gap-3 mt-1">
                             ...
                             {task.status === 'IN_PROGRESS' && timeLeft !== null && (
                                 <span className="text-xs font-mono font-bold text-blue-500 animate-pulse">
                            ...
                            {task.description && <p className="text-sm text-[var(--muted)] hidden md:block border-l border-[var(--border)] pl-3 ml-1">{task.description}</p>}
                        </div>
                    </div>
                </div>
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        {!isAdmin && !task.userId && (
                             <button onClick={() => onAssign(task.id)} className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-lg transition-colors flex items-center gap-2 animate-pulse">
                        ...
                        {!isAdmin && task.userId && (
                            <>
                                {task.status === 'PENDING' && !task.isStopped && (
                                    <button onClick={() => onStart(task.id)} className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-lg transition-colors flex items-center gap-2">
                                        <Play className="w-3 h-3" /> Start
                                    </button>
                                )}
                                {task.status === 'IN_PROGRESS' && !task.isStopped && (
                                     <>
                                        <button onClick={() => onComplete(task.id)} className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-lg transition-colors flex items-center gap-2">
                                ...
                                {task.isStopped && (
                                    <button onClick={() => onStart(task.id)} className="px-3 py-1.5 bg-yellow-600 hover:bg-yellow-700 text-white text-xs font-bold rounded-lg transition-colors flex items-center gap-2">
  ...
  return (
    <div className="glass-card p-6 flex items-center justify-between group hover:scale-[1.02] transition-transform duration-300 relative overflow-hidden">
      ...
      <div className={clsx("relative z-10 p-4 rounded-2xl transition-all duration-300 border border-[var(--border)]", 
          ...
          color === 'emerald' ? 'bg-emerald-500/10 text-emerald-500 group-hover:bg-emerald-500/20 group-hover:scale-110 group-hover:rotate-3 shadow-lg shadow-emerald-500/10' : 
          'bg-rose-500/10 text-rose-500 group-hover:bg-rose-500/20 group-hover:scale-110 group-hover:rotate-3 shadow-lg shadow-rose-500/10'
      )}>
        {icon}
      </div>
    </div>
